...
class account_payment(models.Model):
    ...
    internal_transfer_type = fields.Selection(
        [('a_to_a', 'Account To Account'), ('j_to_j', 'Journal To Journal'), ('j_to_a', 'Journal To Account'),
         ('a_to_j', 'Account To Journal')], string=' Internal Transfer Type', default='a_to_a', copy=False)
    from_account_id = fields.Many2one('account.account', string="From Account", copy=False)
    to_account_id = fields.Many2one('account.account', string="To Account", copy=False)
    to_journal_id = fields.Many2one('account.journal', string="To Journal", copy=False)
    from_journal_id = fields.Many2one('account.journal', string="From Journal", copy=False)
    ...
    def _onchange_journal(self):
        ...
        if self.is_internal_transfer and self.internal_transfer_type in ['a_to_a', 'a_to_j']:
            account_ids = self.env['account.account'].search(
                [('internal_type', 'in', ('receivable', 'payable', 'liquidity', 'other'))])
            if account_ids:
    ...
    @api.onchange('to_account_id', 'to_journal_id')
    def _onchange_to_account_id(self):
        if self.to_account_id or self.to_journal_id:
